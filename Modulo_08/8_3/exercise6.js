...
//onde cada objeto terá o formato { name: nome do aluno, average: media das notas }. P
//ara isso vamos assumir que a posição 0 de notas refere-se ao aluno na posição 0 de alunos, 
...
function studentAverage() {
  const student = alunos.map((string, index) => ({name: string, average: 
    (notas[index].reduce((acc, current) => acc + current, 0) / notas[index].length)
  }));
  return student;
}
